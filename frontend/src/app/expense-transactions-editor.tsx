...
type ExpenseTransactionsEditorProps = {
  apiBaseUrl: string;
  groups: ExpenseCategoryTransactions[];
};
...
function regroupExpenseItems(items: ExpenseTransactionItem[]): ExpenseCategoryTransactions[] {
  const grouped = new Map<string, ExpenseTransactionItem[]>();
...
export default function ExpenseTransactionsEditor({
  apiBaseUrl,
  groups: initialGroups,
}: ExpenseTransactionsEditorProps) {
  ...
  const [groups, setGroups] = useState<ExpenseCategoryTransactions[]>(initialGroups);
  ...
  const startEdit = (rowKey: string, item: ExpenseTransactionItem) => {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const response = editingId
        ? await fetch(`${apiBaseUrl}/api/expense-transactions/${editingId}`, {
            method: "PATCH",
            headers: {
        ...
        : await fetch(`${apiBaseUrl}/api/expense-transactions`, {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
            },
  ...
  const requestExcludedUpdate = async (item: ExpenseTransactionItem, excluded: boolean) => {
    const normalizedId = toSafeId(item.id);

    return normalizedId
      ? fetch(`${apiBaseUrl}/api/expense-transactions/${normalizedId}`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      : fetch(`${apiBaseUrl}/api/expense-transactions`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const toggleExcluded = async (item: ExpenseTransactionItem) => {
    setIsSaving(true);
    setError(null);
    ...
    try {
      ...
      if (!response.ok) {
        const payload = (await response.json().catch(() => null)) as { detail?: string } | null;
        setError(payload?.detail ?? "소비 제외 처리에 실패했습니다.");
        return;
      }
      ...
      if (!item.excluded) {
        setGroups((prevGroups) =>
          prevGroups
            .map((group) => {
              const items = group.items.filter((groupItem) => {
      ...
      setError("소비 제외 처리 중 네트워크 오류가 발생했습니다.");
    } finally {
      setIsSaving(false);
    }
  };

  const excludeWholeCategory = async (group: ExpenseCategoryTransactions) => {
    ...
    try {
      ...
      setGroups((previousGroups) => previousGroups.filter((currentGroup) => currentGroup.category !== group.category));
